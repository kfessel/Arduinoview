...
/*loadjsfile("lib/flot/jquery.js",
           function(){
               loadjsfile("lib/flot/jquery.flot.js",
                          function(){
                              loadjsfile("lib/flot/jquery.flot.navigate.js");
                          });
           });
autoloader("lib/plotter.js");
*/
...
function createStandardelement(x){
    ...
    switch(element){
        ...
        case 'b':
            //b Button _name 
            HTMLappend("<input id=\""+elementID+"\" value=\""+data.value+"\" type=\"button\" onclick=\'sendframe(\"!g"+id+"\")\' style=\""+data.style+"\">")
            var elem=document.getElementById(elementID)
            IDs[id]=function(){ return {element:document.getElementById(elementID)}}();
            break;
        case 's':
            //s slider
            HTMLappend("<input id=\""+elementID+"\" value=\""+data+"\" type=\"range\" min=0 max=255 onchange=\'sendframe(\"!g"+id+"\" + this.value)\' style=\""+data.style+"\">")
            var elem=document.getElementById(elementID)
            IDs[id]=function(){ return {element:document.getElementById(elementID)}}();
            break;
        case 'c':
            //c checkbox
            HTMLappend("<input id=\""+elementID+"\" type=\"checkbox\" onchange=\'sendframe(\"!g"+id+"\" + this.value)\' style=\""+data.style+"\">")
            var elem=document.getElementById(elementID)
            IDs[id]=function(){ return {element:elem}}();
            break;
        case 'M':
        ...
        case 't':
            //text input
            HTMLappend("<input id=\""+elementID+"\" type=\"text\" onchange=\'sendframe(\"!g"+id+"\" + this.value)\' style=\""+data.style+"\">")
            var elem=document.getElementById(elementID)
            IDs[id]=function(){ return {element:elem}}();
            break;
        case 'd':
            HTMLappend("<div id=\""+elementID+"\"  style=\"display:inline-block;"+data.style+"\"></div>")
            var elem=document.getElementById(elementID)
            IDs[id]=function(){ return {element:elem}}();
            break;
